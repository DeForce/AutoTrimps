...
function buyFoodEfficientHousing() {
    ...
    if (bestfoodBuilding) {
        const element = document.getElementById(bestfoodBuilding)
        if (element) element.style.border = "1px solid #00CC01";
...
function buyGemEfficientHousing() {
	...
	if (unlockedHousing[house] == "Gateway" && !canAffordBuilding('Gateway'))
        ...
        const element = document.getElementById(unlockedHousing[house])
        if (element) element.style.border = "1px solid #FFFFFF";
    }
    ...
    for (var best in keysSorted) {
        ...
        if (game.buildings[keysSorted[best]].owned < max || max == -1) {
            ...
            const element = document.getElementById(bestBuilding)
            if (element) element.style.border = "1px solid #00CC00";
            var skipWarp = false;
